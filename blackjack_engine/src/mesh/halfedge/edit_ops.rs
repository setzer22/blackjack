...
pub fn extrude_along_curve(
    ...
    // Start by instancing the cross section in all the points of the backbone curve.
    let mut result_mesh = copy_to_points(backbone, cross_section)?;

    // TODO: This can fail if the curve was edited, since the vertices would not
    // be in the right order. It only works by accident because the default
    // lines follow this property.

    // Classify the half-edges of each cross section using the `instance_idx`
    // channel generated by copy_to_points.
    let mut halfedges_of_instance: Vec<Vec<HalfEdgeId>> =
        vec![vec![]; backbone.read_connectivity().num_vertices()];
    let result_instance_idx_ch = result_mesh
        .channels
        .read_channel_by_name::<HalfEdgeId, f32>("instance_idx")?;
    for (h, _) in result_mesh.read_connectivity().iter_halfedges() {
        let idx = result_instance_idx_ch[h] as usize;
        halfedges_of_instance[idx].push(h);
    }
    drop(result_instance_idx_ch);

    for (h, h2) in halfedges_of_instance.iter().tuple_windows() {
        bridge_chains_ui(&mut result_mesh, h, h2, flip)?;
    }
...
// TODO
// - [ ] Remove the #[lua] annotation after parsing
// - [ ] The thing after borrow_mut is wrong: It's name, should be the type
// - [ ] Inner should return a mlua::Result
// - [ ] Wrapped fn should return an anyhow::Result
// - [ ] At the end of the wrapped fn, there should be a mapping to a lua error
//   for the anyhow error
// - [ ] Optional: Allow non-`Result` return types
// - [ ] When one arg is a reference, it needs to be borrowed on the invoke args
// - [ ] When an arg is a mut reference, its borrowed line needs to use `let mut`
// - [ ] The call to .get for the "ops" table should be like .get::<_, mlua::Table>
 
/*
mod lua_fns {
    use super::*;
    use crate::{prelude::{ChannelValueType, HalfEdgeMesh}, lua_engine::ToLuaError};
    ...
    fn export_add_channel_test_to_lua(lua: &mlua::Lua) {
        ...
        ) -> mlua::Result<HalfEdgeMesh> {
            let mesh = mesh.borrow_mut::<HalfEdgeMesh>()?;
            add_channel_test(&mut mesh, kt, vt).map_lua_err()
        }
