...
pub fn extrude_along_curve(
    ...
    let mut halfedges_of_instance: Vec<Vec<HalfEdgeId>> = vec![vec![]; backbone.read_connectivity().num_vertices()];
    let result_instance_idx_ch = result_mesh.channels.read_channel_by_name::<HalfEdgeId, f32>("instance_idx")?;
