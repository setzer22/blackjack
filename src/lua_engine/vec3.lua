...
local function new(x, y, z) return setmetatable({x = x, y = y, z = z}, vec3) end

-- check if an object is a vec3
local function isvec3(t) return getmetatable(t) == vec3 end

-- meta function to add vec3s
function vec3.__add(a, b)
    assert(isvec3(a) and isvec3(b),
           "add: wrong argument types: (expected <vec3> and <vec3>)")
...
function vec3.__sub(a, b)
    assert(isvec3(a) and isvec3(b),
           "sub: wrong argument types: (expected <vec3> and <vec3>)")
    return new(a.x - b.x, a.y - b.y, a.z - b.z)
end
...
local module = {new = new, isvec3 = isvec3}
