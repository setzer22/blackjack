...
/// Performs linear subdivision on this mesh. Each face is subdivided by adding
/// a new vertex on its center, then creating new edges between the halfpoint of
/// each edge and the new center point. This is equivalent to blender's
/// "Subdivide" operation under the "Edge" menu.
///
/// Due to the way this method is implemented, this returns an entirely new
/// mesh, instead of mutating the original one.
pub fn linear_subdivision(mesh: &HalfEdgeMesh) -> Result<HalfEdgeMesh> {
    /// An identifier for the different kinds of vertices in the new mesh
    #[derive(Debug, Clone, Copy, Hash, PartialEq, Eq)]
    enum Id {
        /// Vertices in the original mesh
        OriginalVertex(VertexId),
        /// Vertices generated by splitting an edge at its midpoint
        EdgeMidpoint(VertexId, VertexId),
        /// Vertices generated by creating a vertex at a face midpoint
        FaceMidpoint(FaceId),
    }

    impl Id {
        /// Makes sure (a,b) and (b,a) map to the same Id
        fn edge_midpoint((a, b): (VertexId, VertexId)) -> Self {
            if a < b {
                Self::EdgeMidpoint(a, b)
            } else {
                Self::EdgeMidpoint(b, a)
            }
        }
    }

    /// Maps [`Id`]s to indices in the generated polygon soup
    #[derive(Default)]
    pub struct Indexer {
        next_idx: usize,
        mapping: HashMap<Id, usize>,
        positions: Vec<Vec3>,
    }

    impl Indexer {
        fn index_of(&mut self, id: Id, position: Vec3) -> usize {
    ...
    let mut soup_polygons = Vec::<SVec<usize>>::new();

    for (face, _) in mesh.iter_faces() {
        let vertices = mesh.at_face(face).vertices()?;
        let n = vertices.len();
        ...
        let face_midpoint: Vec3 = positions
            .iter_cpy()
            .reduce(|a, b| a + b)
            .map(|m| m / n as f32)
            .expect("We skip faces with 0 vertices");

        let edge_midpoints = positions
            .iter_cpy()
            .circular_tuple_windows()
            .map(|(a, b)| (a + b) / 2.0);

        let orig_vertex_ids = vertices.iter_cpy().map(Id::OriginalVertex);

        let edge_midpoint_ids = vertices
            .iter_cpy()
            .circular_tuple_windows()
            .map(Id::edge_midpoint);

        let face_midpoint_idx = indexer.index_of(Id::FaceMidpoint(face), face_midpoint);

        let perimeter = edge_midpoints
            .interleave(positions.iter_cpy().cycle().dropping(1).take(n))
            .collect_svec();
        let perimeter_ids = edge_midpoint_ids
            .interleave(orig_vertex_ids.cycle().dropping(1).take(n))
            .collect_svec();

        let pn = perimeter.len();

        for i in (0..pn).step_by(2) {
            let a_pos = perimeter[i % pn];
            let a_idx = indexer.index_of(perimeter_ids[i % pn], a_pos);

            let b_pos = perimeter[(i + 1) % pn];
            let b_idx = indexer.index_of(perimeter_ids[(i + 1) % pn], b_pos);

            let c_pos = perimeter[(i + 2) % pn];
            let c_idx = indexer.index_of(perimeter_ids[(i + 2) % pn], c_pos);

            soup_polygons.push([a_idx, b_idx, c_idx, face_midpoint_idx].into());
        }
    }

    HalfEdgeMesh::build_from_polygons(&indexer.positions, &soup_polygons)
}
